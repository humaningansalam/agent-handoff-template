...
- `repoctl context query` uses the persistent index, overlays only changed paths, and groups source documents, Graph evidence, task receipts, and reviewed knowledge into actionable bundles with typed follow-up actions. Its `project_knowledge` summary reports documents, task history, and reviewed reusable records as separate lanes; zero reviewed records never means the document knowledge base is empty.
