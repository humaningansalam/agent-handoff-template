...
One Candidate query owns the active Discovery episode. The same query accumulates Reviewed files, notes, and selected results; a distinct query retains Chosen scope while replacing those earlier episode values. Replace the Chosen set with `--replace-chosen ... --reason "..."` only when edit scope changes.
When a specific Context or Graph result materially informed the selection, optionally copy `producer`, `result_id`, and one exact `{authority, ref}` tuple from its public `data.result_receipt` into `task discovery add`. repoctl persists the validated typed request and episode identity; do not invent a digest/ref or select `--full`-only diagnostics. This is provenance, not mandatory feature-use logging.
