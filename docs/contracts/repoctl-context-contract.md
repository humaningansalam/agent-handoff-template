...
In `auto`, `reviewed_knowledge` accepts exact record matches as well as records structurally linked to selected paths. `related_history` is retrieved in its own lane. A typed task/artifact identity may resolve its exact Graph task node directly; otherwise a task candidate whose canonical query-term breadth reaches the query's saturating breadth must also be visible in fresh history around the current Graph anchors. At most the first eligible query-retrieved task may add `history_corroboration` to 24 current same-repository `task_working_tree` changed paths. Natural-language corroboration requires independent current lexical query evidence and may promote a path only when the task contributes canonical query evidence absent from the leading current anchor or the path is not lexically dominated by that anchor. Exact typed task lookup may project its current owned paths without requiring the task ID to appear in source text. Corroboration may break a current-source tie and let an existing typed test relation follow the resulting owner; it is not a Graph seed/relation, authority, Chosen file, or edit scope. Weak partial task matches remain history-only, while stale, deleted, cross-repository, `range_observed`, or otherwise unowned paths cannot corroborate current code. Explicit historical modes may retrieve a broader history lane without weakening these code-projection gates.
