...
`repoctl context query` returns a non-authoritative evidence bundle for one explicit product repository. It never mutates product source, task scope, or project authority; after a successful query it atomically stores one regenerable result receipt under `.repoctl-state/result-receipts/**` so later Discovery input can prove membership in the exact compact surface.
...
The command envelope also returns `data.result_receipt` with the producer, canonical typed request, result ID, receipt digest, and bounded selectable `{authority, ref}` entries. Request and selectable fields retain their declared JSON types instead of coercing numbers or other values to strings. Legal Git path characters are encoded by the machine-record serializer rather than rejected because of Markdown delimiters. Receipt reads and writes reject a receipt file or parent path that is a symlink or resolves outside the workspace. `--full` never widens membership: every renderer uses the same compact producer projection. Repeating an identical result is byte-idempotent; the same producer/repository/result identity with a different request or membership fails with `result_receipt_conflict` instead of overwriting evidence.
...
Compact groups merge chunks with the same path into one file-level item and preserve their locations in a `sections` list. A global budget returns at most eight actionable items, normally three to eight when enough evidence exists, with small per-lane limits so authority, source, test, history, and Knowledge do not compete in one ranking. Source compaction first selects its bounded likely-owner set. Path, path-area, and path-scope matches are ranking evidence only and never exclude another candidate; only an explicit typed scope constraint may hard-scope retrieval. Fresh typed Graph connectivity ranks before provider identity, structured path or section evidence, and body-only vocabulary. Connectivity found during preselection remains available to compact selection even when that relation is not projected into the displayed seed neighborhood. `CALLS`, `USES_FILE`, and source-directed `TESTS_FILE` may establish owner coherence; `IMPORTS_FILE` remains traversal evidence and cannot reserve an owner slot by itself. Connectivity is evaluated from either endpoint of the same typed relation. In the default three-source bound, compaction preserves direct-owner diversity and uses the final source slot for a connected structural locus only when the retained sources do not already represent one; narrower two-source modes do not discard a second direct owner to manufacture that reserve. Graph seeds that satisfy the owner contract retain bounded owner capacity, and candidates from a different structural locus survive when a typed relation connects them to a retained owner. An unconnected direct source that contributes a distinct structural or query-field identity remains eligible; compaction does not require every plausible owner to belong to one closure. Body-only candidates consume a compact owner slot only when they add an unrepresented query concept. A lexical Graph seed remains navigation evidence and does not outrank a stronger current direct match merely because it traversed. The single default test slot follows the first retained owner: an exact `CALLS` relation and then ordinary typed connectivity to that owner rank first; shared canonical query evidence resolves otherwise equivalent connections. Only when no test connects to the first owner does connectivity to another retained owner, followed by lexical affinity, fill the slot. Every current source/test/declared-config profile remains eligible; directory names such as example, frontend, backend, src, lib, and test do not create source-owner policy beyond the typed test boundary. Exact structured-data identities use their own bounded lane; ordinary structured-data body matches never enter this cohort. Repeated vocabulary or the same term echoed across several fields cannot accumulate unbounded selection value. A weak isolated singleton can remain an explicit omission instead of consuming a compact slot when it adds no lane, role, component, exact identity, or Graph-supported evidence. Redundant lexical consumers may be omitted from the compact view, but full evidence keeps every hypothesis for diagnostics. Text-only and Graph-unsupported files use the same lexical coverage path. When matching authority and procedure documents both exist, compact `must_read` reserves one of each before filling its remaining slots. Eight items is a maximum, not a quota: projection stops instead of filling remaining slots with redundant lexical noise. Each item carries its selection reason; typed continuations are deduplicated at bundle level. Fresh resolved anchors additionally produce at most three typed `graph_seed_refs`, each carrying the exact source digest, anchor, provenance, strength, and file/symbol continuation. These refs and their embedded continuations sit outside item and top-level continuation budgets; they are not duplicated into ordinary continuation fill and do not become Chosen, authority, or required edit scope. After source/test compaction, one final navigation projection preserves actual seed membership and the original exact, explicit, strong, and Reviewed-Knowledge-derived current-source prefix. It stable-partitions only the remaining weak seeds: the compact primary leads when it is an actual seed, seeds connected to retained sources by fresh resolved `CALLS`, `USES_FILE`, or source-directed `TESTS_FILE` endpoints follow, and other weak seeds keep their relative order. `IMPORTS_FILE` remains a continuation and cannot promote a weak seed. In `auto`, the one relation slot accepts only a fresh resolved relation whose `from_path` or `to_path` touches the compact primary, then another retained source; traversal `origin_paths` do not establish endpoint incidence. The slot remains empty when no such relation exists. That final resolution feeds compact/full completeness, `graph_seed_refs`, human views, and Task Pack; no display-only reorder, synthetic seed, or second traversal is allowed. `graph_anchor.identity_coverage` becomes typed `partial` when a resolved anchor cannot retain that identity. `graph_anchor.selection_coverage` describes only the bounded seed set. Separate root-level `working_set_coverage` describes the source/test items actually visible after per-lane and global compact bounds, so a complete anchor set cannot hide a partial displayed working set. Full selection diagnostics retain every eligible, selected, omitted, and coverage-omitted path; compact diagnostics bound those path lists without flattening field identity.

When one path has both direct query evidence and Graph relations, its compact primary role remains `change_candidate` or `test_candidate`; provider-confirmed relationships remain available through relation evidence and typed continuations. A weak provider-symbol section receives symbol-specific Graph corroboration only when that exact provider symbol node ID is the `from_id` or `to_id` of a fresh resolved `CALLS` relation among the current candidates. Otherwise the current file remains eligible only as its lexical file anchor. Early ranking attaches a `CALLS` relation to a provider-symbol section only under that same endpoint identity, and attaches a structured-file relation to a provider-relationship section only when its `source_fact_id` is present in the relation evidence. `file_impact` deliberately uses the file as the bounded relation context. Other file-wide relations remain visible in the separate relation lane but cannot make an unrelated lexical section appear connected. A test reached directly from the first non-test Graph anchor also carries `anchor_connected_test`, which ranks ahead of a lexical test seed that is connected only to another dependency. A path reached only through Graph keeps its dependency role.

The first unfamiliar query is a ranked, provenance-bearing hypothesis rather than an oracle result. A later related query may improve when current source/Graph evidence changes, a validated completion receipt links a strongly matched task to its current owned paths, or an explicitly Reviewed Knowledge record contributes a validated current-source applicability path. These lanes remain separate and never auto-create Graph facts, authority, Knowledge approval, Chosen files, or edit scope.
...
In `auto`, `reviewed_knowledge` accepts exact record matches as well as records structurally linked to selected paths. `related_history` is retrieved in its own lane. Task and artifact candidates are retrieved independently of neighborhood traversal. Context distinguishes an exact typed task or artifact identity from a strong natural-language task match and asks Graph's typed task-path lookup only for current owned paths. For natural-language recovery, Context loads and ranks current source at those paths and uses only the first strong task with at least one eligible current lexical owner. At most 24 paths from that task may receive `history_corroboration`, and only when the path independently matches the query and either the task adds canonical query evidence absent from the leading current anchor or the path is not lexically dominated by that anchor. This provenance may influence bounded owner selection, so the current file may later become an ordinary lexical Graph anchor; the receipt itself is not a Graph edge, authority, Chosen file, or edit scope. Exact typed task or artifact lookup may expose at most 24 current owned paths per matched task without requiring the task ID in source text; those paths carry `exact_task`, not `history_corroboration`, are added after anchor and traversal selection, and cannot change Graph seeds or traversal. Eligible ownership requires a current, non-deleted, same-repository `TASK_CHANGED_FILE(role=path)` target. Weak partial matches remain history-only, while stale, deleted, cross-repository, old-path, `range_observed`, or otherwise unowned paths cannot project current code. Explicit historical modes may retrieve a broader history lane without weakening these code-projection gates.
...
Retrieval query text comes from the active Discovery episode's single Candidate query. A distinct new query starts a new episode, clears earlier Reviewed/result/note evidence, and carries current Chosen scope forward. A selected Context receipt supplies that exact query and owns the episode identity; same-query modes and typed Graph follow-up selectors accumulate without changing the episode. Goal and Handoff prose are not parsed as symbols. A test is directly connected only through explicit Discovery evidence, a provider-confirmed relation, or manifest mapping.
...
Task Pack schema version 4 carries the active query, Reviewed and Chosen sets, notes, selected-result provenance with its validated typed request and episode identity, and the exact Context-produced `graph_seed_refs`. Full JSON retains those exact inputs. Default compact JSON bounds query and note text; selected-result provenance keeps its stable IDs, authority and ref plus a typed `request_preview` and a `request_digest` of the exact request, so free-form input cannot silently defeat the compact output budget. Result receipts are checked only when a new selection is recorded; deleting a regenerable receipt later does not invalidate task Markdown or a Pack derived from that recorded evidence. Legacy selected-result records remain historical without guessing a missing request.
