...
This document describes the implemented Context query and Task Pack surface. Ordinary queries use the persistent current-evidence index and stale-path overlays without consulting task outcomes. Outcomes stay in immutable completion evidence or their explicit lifecycle stores; `past_decision` and `failure_mode` expose bounded cold-history matches only in `related_history`. Context also projects declared, set-aware component membership and confirmed crossings from the current Graph snapshot. Stored producer result receipts remain flat; Context exposes a separate versioned public projection, and Discovery freezes canonical capsules only for explicitly selected manifest members.
...
Context never triggers a hidden Graph build. With a healthy materialization, queries read the persistent SQLite evidence index and overlay only changed paths. If Graph is absent, Context returns a typed partial result from live source, declared config, structured data, document, task, and Knowledge evidence. Unreadable, incompatible, incomplete, or digest-mismatched persisted Graph state remains a typed failure. Graph relations are marked unavailable, while JSON `next_actions` use the structured cause code to return the required `graph build` or `graph build --rebuild` command plus a command that resumes the same Context query. When a Graph snapshot remains usable and only indexed source evidence is degraded, the original typed evidence warning remains visible and is not relabeled as `context_graph_unavailable`. The fallback never repairs Graph/index state; a successful result writes only its regenerable result receipt.

On a fresh adoption, readable current source is sufficient for a successful Context query. Absent `.repometa`, Graph materialization, completion history, and Reviewed Knowledge are independent optional enrichment gaps reported through warnings and completeness; their absence never initializes or rebuilds those stores implicitly. Malformed or inconsistent persisted state remains a typed trust-boundary failure.
...
In `auto`, `reviewed_knowledge` accepts exact record matches as well as records structurally linked to selected paths. Completion outcomes never enter the current-candidate ranking or Graph-anchor selection. Context scans cold completion history only for the explicit `past_decision` and `failure_mode` modes.
...
`task start` and `task show` never create or replace a binding. The machine binding receipt stores exact Handoff and structured Task/repository input digests plus the optional Pack path, artifact digest, and input digest; it does not interpret or certify Handoff prose. Task creation places exactly one `<!-- repoctl: generated-handoff -->` marker in the generated Handoff. A marked placeholder is readable with a typed warning but cannot be bound; removing the marker and supplying the four valid fields makes it bindable. Repoctl never classifies authorship by comparing prose with current or historical templates. A legacy unmarked live Handoff requires one explicit bind, and a legacy v3 receipt remains readable but inactive until a v4 rebind. `task show --summary --json` reports only `current | inactive | historical`, and only `current` is freshness-active. `task resume --json` owns workspace discovery as `no_live | single_live | ambiguous`; `task resume <TASK_ID> --json` explicitly selects one returned live task without writing selection state. Neither form falls back to archived history, and both preserve the selected task plus typed unhealthy health even when repository layout parsing fails. They return current prose as `readable_handoff`; they return the same prose as `executable_handoff` only when repository lifecycle health is also executable. Context history and its `task.show` continuation remain inspection evidence, not resume selection. A malformed receipt or live Handoff fails closed. Archived Handoffs remain readable historical evidence.
...
Task packs do not query reviewed knowledge or completion history. Both indexed and live-fallback retrieval exclude those source kinds before ranking, and Pack construction does not enable explicit follow-up reuse. Use an explicit historical mode or Context Doc when a Pack needs durable historical context.
