...
`task.doctor` and `task.finish` report current changed-Chosen structured-check coverage under `data.structured_verification`, including status and required, passed, missing, and nonpassing counts. While a task is live, doctor owns the complete union under `data.action_inputs.unverified_chosen_subjects` and may partition it into `missing_structured_verification_subjects` and `nonpassing_structured_verification_subjects`. Only the missing partition emits a `task_verification_add` action. A nonpassing current-version status set requires evidence review because appending a passed record does not erase prior failed, mixed, or blocked outcomes. Finish adds a bounded `unverified_subjects` audit list with its count and truncation flag; its missing/nonpassing warnings never offer a mutation command because the task is already immutable.
