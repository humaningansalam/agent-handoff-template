...
Discovery records explicit exclusions and selected flat result entries, validates them against the producer receipt, and freezes canonical member capsules into task-owned outcome state. Structured verification and completion receipt outcome capture, catalogue ingress, bounded hot projection, and the ordinary Context current-subject join are implemented. Producer result receipts remain flat rather than exposing canonical members directly. Exact Graph task/artifact lookup and Context `past_decision` / `failure_mode` search are explicit cold-history operations; neither widens the ordinary hot projection.
...
Task finish freezes the canonical episode projection and structured verification records inside the immutable completion receipt. Receipt validation proves the task-owned structured state, referenced evidence, task artifact, and repository identity agree; Markdown prose is not a second structured-state owner.
