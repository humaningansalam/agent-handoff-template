...
Discovery records explicit exclusions and selected flat result entries, validates them against the producer receipt, and freezes canonical member capsules into task-owned outcome state. Structured verification, completion receipt outcome capture, catalogue ingress, and bounded hot projection are implemented. Stored producer result receipts remain flat rather than exposing canonical members directly; Context's versioned public projection does not redefine that manifest. Exact Graph task/artifact lookup and Context `past_decision` / `failure_mode` search are explicit cold-history operations; ordinary Context does not consume completion outcomes.
...
-> explicit history or exact task/artifact inspection in a later query
```
...
For each changed current Chosen product file, readiness uses the last appended verification record bound to that exact subject version. Missing verification remains advisory and may point to `task verification add`; any latest nonpassing record blocks doctor readiness and finish until a later passed record for the same version recovers it. Records for other subjects or versions do not supersede current evidence. The append-only history remains intact in completion audit output, including repeated records with identical content.
...
Ordinary Context does not consume completion outcomes. `past_decision` and `failure_mode` are explicit cold-history selectors, and exact Graph `--task` and `--artifact` selectors inspect one validated historical record. These paths expose provenance without adding current candidates, changing ranking, creating Graph seeds or relations, or inferring scope.

`past_decision` and `failure_mode` are the explicit Context cold-history selectors. Their bounded matches appear only in `related_history`, after current candidate retrieval, ranking, Graph-anchor selection, and traversal are fixed. They do not add or reorder current candidates, create Graph seeds or relations, or infer scope. Exact Graph `--task` and `--artifact` selectors likewise build an ephemeral projection from the one validated cold record rather than inserting cold history into the active snapshot.

Outcome roles remain inspectable in immutable completion evidence. Excluded, stale, deleted, renamed, unreadable, and unrecorded subjects never suppress independently retrieved current source.
...
Outcome capture reuses the existing Discovery and finish mutation boundaries. It does not require per-query, per-click, per-file-open, or feature-use logs.

The immutable receipt keeps the complete canonical outcome. The hot projection coalesces claims for retained file versions so Graph can expose bounded task/file history without scanning cold receipts. Context uses completion evidence only through explicit cold-history modes. Compression may remove repeated presentation but never turn omission into a negative judgment.
...
Catalogue search vocabulary is a bounded ingress projection of task title/slug, changed paths, and human-authored Discovery, Verification, or legacy Failure sections. It never tokenizes the canonical receipt JSON or the complete archived task, so machine schema keys, digests, timestamps, and numeric fragments do not become history matches merely because they occur in serialization metadata. A projector-version change invalidates the derived catalogue until explicit history rebuild; it never rewrites completion receipts or archived tasks.

Only `file` outcomes are eligible for the bounded hot outcome frontier used by Graph task/file history. The stable role key is `file:<repo-relative-path>`, while the hot cell key additionally binds the file subject's version digest. That digest covers canonical path identity, content, and retrieval classification. Old cells are bounded derived history and never become ordinary Context candidates.

Graph snapshot identity remains lineage provenance, not file freshness. Document, symbol, relationship, task, artifact, and Knowledge subjects remain in the immutable receipt or their existing explicit lifecycle stores. A changed-entry witness for a versioned file outcome is retained in that versioned cell. Only a legacy or no-outcome changed path uses a bounded unversioned `file:<path>` cell so Graph can project structured task/file history without treating that witness as a learned outcome.
...
Task finish work is proportional to the current task's bounded catalogue event. Normal materialization is proportional to new catalogue entries and the cells named by those entries. Ordinary Context query work and payload do not depend on completed-task count. Only explicit history, exact lookup, audit, or recovery reads completion history. Cold storage may grow by one completion envelope per task; repeated large payloads are content-addressed or referenced rather than copied. Permanent lossless audit and bounded total disk cannot both be promised, so any destructive cold-history retention/export policy is explicit workspace policy.
...
Any future automatic outcome-reuse proposal is accepted only when field-repository scenarios show all of the following. The current release does not implement that proposal because Termroom and Areum replay failed the cost and working-set criteria:
