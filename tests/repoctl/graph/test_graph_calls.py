...
def test_graph_query_symbol_callers_and_callees(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert main(["graph", "query", "--file", "auth/flow.py", "--json"]) == 0
    file_result = json.loads(capsys.readouterr().out)["data"]["result"]
    continuation = next(item for item in file_result["continuations"] if item["selector"].get("value") == "validate_token")
    assert continuation["selector"] == {"kind": "symbol", "value": "validate_token", "in_file": "auth/flow.py"}
    assert continuation["query_types"] == ["symbol", "callers_of", "callees_of", "impact_symbol"]
    ...
    selector = continuation["selector"]
    assert main(["graph", "query", "--callers-of", selector["value"], "--in-file", selector["in_file"], "--json"]) == 0
